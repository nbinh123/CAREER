...
// [UI] Chuyển đổi MenuPage.js gốc (CRA + Tailwind thuần, dùng chung
// Btn/Modal/FormInput/ImageUploadField/IngredientPicker). Giữ nguyên 100%
// logic nghiệp vụ: CRUD món ăn chạy "local-first" qua useFoodZustand y hệt
// IngredientsPage.js (thêm/sửa/xoá/toggle trạng thái chỉ cập nhật state cục
// bộ + đánh dấu pendingChanges, chỉ gọi API thật khi bấm "Lưu N thay đổi");
// normalizeFood/safeFmtVND giữ nguyên 100% (thuần JS, không đụng DOM); giá
// vốn tự tính từ tổng cost của nguyên liệu đã chọn khi có ≥1 nguyên liệu,
// ngược lại cho nhập tay; loại trừ danh mục "Trái cây mix" khỏi danh sách
// hiển thị (quản lý riêng ở FruitPage); "Làm mới" (refreshCosts) bị khoá
// khi còn thay đổi chưa lưu; xuất/nhập JSON qua endpoint /api/foods.
//
// Khác biệt platform:
//   - "../components/Button" / "FormInput" / "Modal" (dùng chung bên web,
//     không có mã nguồn gốc) → dựng lại 4 helper cục bộ NGAY TRONG FILE NÀY
//     (ActionBtn/IconBtn/FieldInput/ModalOverlay), ĐÚNG cách IngredientsPage.js
//     đã làm — không tách thành component dùng chung dù đây là lần dùng lại
//     thứ 2 trong dự án, để nhất quán với quyết định đã ghi nhận ở
//     IngredientsPage.js (chưa có đối chiếu variant gốc nên chưa muốn "chốt
//     cứng" thành API dùng chung; gộp lại thành 1 lần refactor sau khi có
//     nhiều trang hơn để đối chiếu là hợp lý hơn). primary/secondary/outline
//     dùng đúng màu đã chọn ở IngredientsPage.js (primary=green-600,
//     secondary=trắng viền xám, outline=viền mờ hơn) — MenuPage.js gốc CÓ
//     bằng chứng trực tiếp cho quy ước này: các nút không truyền variant
//     ("Thêm món mới", "Xác nhận") dùng biến thể mặc định của <Button>,
//     còn "Sửa"/"Chi tiết"/"Làm mới"/"Xuất JSON"/"Tải lên JSON" đều truyền
//     variant="secondary" tường minh, và "Hủy" truyền variant="outline" —
//     khớp 100% với quy ước primary=mặc định đã chọn ở IngredientsPage.js.
//   - "../components/ImageUploadField" / "IngredientPicker" — KHÔNG giống
//     Button/FormInput/Modal, đây là 2 component nghiệp vụ thật sự (không
//     phải UI-kit cơ bản), bản web gốc cũng tách file riêng cho chúng, nên
//     dựng lại thành component RN dùng chung ở src/components/ (thay vì
//     inline) — xem chi tiết giả định về công thức cost = quantity ×
//     pricePerLargeUnit ở đầu file src/components/IngredientPicker.js
//     (không có mã nguồn IngredientPicker.js gốc để đối chiếu 1-1).
//   - <select> danh mục (CAT_OPTIONS cố định, 6 mục) → dự án chưa có
//     @react-native-picker/picker (không có trong package.json, không tự
//     ý thêm dependency mới) → chuyển thành dãy pill chọn 1 trong 6, cùng
//     kiểu với bộ lọc CAT_FILTER phía trên (đã có sẵn UI pill tương tự
//     ngay trong chính trang này).
//   - Grid responsive 1/2/3/4 cột (Tailwind) → RN mobile chỉ còn đúng
//     breakpoint mặc định (không prefix) = 1 cột, nên list chuyển thành 1
//     cột full-width, ĐÚNG NHƯ chính bản gốc đã định nghĩa cho màn hình hẹp
//     (không phải suy diễn thêm) — khác quyết định "table → card dọc" ở
//     IngredientsPage.js/Customers.js (vốn áp dụng cho <table>), ở đây
//     MenuPage.js gốc vốn đã là card-grid nên giữ nguyên bố cục card (ảnh
//     trên, nội dung dưới), chỉ đổi số cột.
//   - <input type="checkbox"> (AvailabilityToggle kiểu dáng toggle switch +
//     checkbox "Đang bán" trong form) → RN Switch, đúng quy ước đã dùng ở
//     IngredientsPage.js cho needContinuousRestock.
//   - 3 field số costPrice/originalPrice/aiTrainingWeight trong form thêm/
//     sửa: bản gốc ép `+e.target.value` ngay mỗi lần gõ (an toàn trên web
//     vì <input type="number"> tự giữ buffer hiển thị riêng). RN TextInput
//     không có buffer này — giữ CHUỖI trong state `form` lúc đang sửa (đúng
//     pattern NUMERIC_FIELDS đã dùng ở IngredientsPage.js), chỉ ép sang
//     Number ở handleSave/các chỗ tính margin trực tiếp; EMPTY_FOOD vì vậy
//     đổi 3 field này thành "0" (chuỗi) thay vì 0 (số) — KHÔNG đổi logic
//     nghiệp vụ, chỉ đổi kiểu lưu trữ tạm trong lúc sửa.
//   - Số lượng nguyên liệu trong công thức cũng giữ dạng chuỗi lúc gõ vì
//     cùng lý do trên — xử lý bên trong IngredientPicker.js, MenuPage.js
//     chỉ cần ép Number ở handleSave trước khi stage lên store (tổng
//     computedCostPrice đã luôn là Number vì IngredientPicker tự tính cost
//     bằng Number ngay khi thay đổi, không đợi tới lúc lưu).
//   - <input type="file"> để chọn JSON import + fileInputRef ẩn → bỏ hẳn,
//     dùng thẳng pickJSONFile() (expo-document-picker) rồi importJSON(),
//     đúng pattern handleImportPick đã dùng ở IngredientsPage.js.
//   - Loader2 (icon xoay CSS animate-spin) → ActivityIndicator, RefreshCcw
//     lúc "Làm mới" cũng vậy — đúng quyết định đã ghi nhận nhiều lần
//     (README/CashFlow.js/Customers.js/IngredientsPage.js).
//   - Lưới skeleton 8 ô lúc loading lần đầu → SkeletonCard dùng Reanimated
//     withRepeat/withTiming pulse opacity, đúng kỹ thuật SkeletonCard đã
//     dựng ở Customers.js (thay vì chỉ 1 ActivityIndicator như
//     IngredientsPage.js, vì bản gốc MenuPage.js có hẳn 1 lưới skeleton
//     card thật sự, không chỉ là dòng loading đơn giản).
//   - FoodImage: bg-gradient-to-br (CSS) khi thiếu ảnh/lỗi tải → LinearGradient
//     (expo-linear-gradient), đúng cách Avatar ở Customers.js đã xử lý
//     gradient nền + chữ cái đè lên trên.
//   - title="..." (tooltip HTML) trên nút "Làm mới" khi có pendingChanges →
//     bỏ hẳn, không có API tương đương phổ biến trong dự án; trạng thái
//     disabled + độ mờ đã đủ truyền đạt, giữ nguyên phần LOGIC disabled.
//   - saveStatus "saved" và refreshMsg: bản gốc hiện dòng chữ inline ngay
//     cạnh các nút trong toolbar (không phải toast nổi) — GIỮ ĐÚNG vị trí
//     inline này (khác CashFlow.js dùng toast nổi đáy màn hình cho
//     updateMsg, vì đó là quyết định bố cục riêng của từng trang gốc, ở
//     đây bản gốc đã chọn rõ ràng đặt cạnh nút, không tự ý đổi bố cục).
