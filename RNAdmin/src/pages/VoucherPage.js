// src/pages/VoucherPage.js
// [UI] Chuyển đổi VoucherPage.jsx gốc (CRA + Tailwind thuần, chưa dùng
// component dùng chung nào — form tự viết tay). Giữ nguyên 100% logic
// nghiệp vụ: fetch stats theo khoảng thời gian (all/today/7d/30d), fetch +
// debounce tìm kiếm danh sách (350ms), lọc theo status virtual field server
// trả sẵn (KHÔNG thêm query param vì backend chưa hỗ trợ lọc theo status
// tính toán — lọc ngay trên client), tạo/sửa voucher qua modal, bật/tắt
// nhanh isActive từ danh sách, validate bắt buộc tên/mã/giá trị giảm/ngày
// kết thúc trước khi submit, và toàn bộ helper thuần JS (fmtVND, splitCsv,
// toDateInputValue, formatDateVN — không đụng DOM) copy y hệt. Dữ liệu
// "áp dụng cho" (danh mục/món ăn) vẫn lấy từ useFoodZustand đúng như bản
// gốc, kể cả ghi chú "categories" của store luôn rỗng nên phải suy category
// trực tiếp từ categoryId có sẵn trên các món đã fetch.
//
// Khác biệt platform:
//   - <table> 9 cột (Mã/Tên/Loại/Giá trị/Đơn tối thiểu/Đã dùng/Thời gian/
//     Trạng thái/Thao tác) → RN không có bảng, màn hình ~380px không đủ chỗ
//     hiển thị ngang → mỗi voucher chuyển thành 1 VoucherCard dọc, đúng
//     tinh thần IngredientsPage.js/Customers.js/StoragePage.js đã làm.
//     KHÔNG bớt field nào, chỉ gộp "Loại" + "Giá trị" thành 1 dòng.
//   - <select> "Loại giảm" (PERCENTAGE/FIXED) → RN không có <select> gốc,
//     đổi thành 2 nút bấm dạng segmented (cùng cách StoragePage.js đã dùng
//     cho bộ lọc loại giao dịch IMPORT/DAMAGE).
//   - SearchableMultiSelect gốc (dropdown tuyệt đối định vị, đóng khi click
//     ra ngoài bằng document.addEventListener — API chỉ có trên web) → RN
//     không có z-index/click-outside kiểu DOM, và dropdown lơ lửng bên
//     trong 1 modal đã cuộn (ScrollView lồng ScrollView) rất dễ vỡ layout.
//     Đổi thành 1 "chế độ picker" ngay trong modal thêm/sửa: bấm vào ô
//     "Danh mục áp dụng"/"Món ăn áp dụng" → nội dung modal đổi hẳn sang
//     danh sách checkbox có ô tìm kiếm (PickerBody) + nút "Xong" quay lại
//     form, đúng pattern IngredientPickerBody đã dùng ở StoragePage.js
//     (biến `pickerMode` đóng vai trò tương đương biến `mode` bên đó).
//     Nút X ở header modal LUÔN đóng hẳn cả modal (kể cả đang ở picker),
//     giữ đúng hành vi đã có ở StoragePage — quay lại form chỉ qua nút
//     "Xong" trong PickerBody, không qua X.
//   - <input type="checkbox"> (kênh áp dụng) → RN không có checkbox gốc,
//     tự vẽ 1 ô vuông 18x18 (viền xám/nền xanh + icon Check khi chọn),
//     dùng lại cho cả kênh áp dụng lẫn từng dòng trong PickerBody.
//   - <input type="date"> → DateField dùng
//     @react-native-community/datetimepicker, copy nguyên cách StoragePage
//     đã dựng (Android tự đóng picker sau khi chọn, iOS dạng spinner + nút
//     "Xong"), kể cả cách tính ISO date theo giờ LOCAL (không dùng
//     toISOString() vì lùi 1 ngày ở múi giờ VN +7).
//   - ChevronDown ở nút mở dropdown gốc (xoay 180° khi mở) → đổi thành
//     ChevronRight tĩnh, vì giờ bấm vào không "mở rộng tại chỗ" nữa mà
//     chuyển hẳn sang màn picker khác (đúng hơn về mặt affordance).
//   - Nút Power (bật/tắt nhanh) gốc dùng hover:bg-rose-50/hover:bg-green-50
//     để gợi ý hành động sắp làm — RN không có hover, nên tô màu tĩnh theo
//     đúng hướng hành động (đang bật → viền/icon rose vì bấm sẽ TẮT; đang
//     tắt → viền/icon xanh vì bấm sẽ BẬT), cùng tinh thần nút Trash2 ở
//     IngredientsPage.js luôn tô đỏ vĩnh viễn thay vì chỉ khi hover.
//   - 2 cột ngang (grid-cols-2) cho "Mã voucher"/"Tên voucher" trong form
//     gốc → xếp dọc toàn bộ chiều rộng vì tên voucher/placeholder tiếng
//     Việt khá dài, 2 cột ngang trên màn ~380px sẽ vỡ chữ; các cặp field
//     ngắn hơn (đơn tối thiểu/giảm tối đa, tổng lượt dùng/lượt mỗi khách)
//     vẫn giữ 2 cột (47% mỗi ô) đúng cách IngredientsPage.js đã làm.
//   - CheckCircle2/Hourglass/Ban: tên icon "họ Circle-suffix" kiểu này đã
//     bị đổi tên ở lucide 1.0+ (CheckCircle2 → CircleCheckBig — xem ghi
//     chú icon ở README). package.json ở đây pin "^0.383.0" (bản 0.x, TRƯỚC
//     mốc đổi tên 1.0, và caret trên 0.x chỉ cho phép patch nên sẽ không
//     tự nhảy qua 1.0), nên tên cũ vẫn dùng được — nếu sau này nâng cấp
//     lucide-react-native qua major 1, nhớ đối chiếu lại đúng như README
//     đã dặn (đừng suy đoán 1-1 từ tên bên web).
