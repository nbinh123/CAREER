...
export default function Chart08({ data: rawData = [], tf = "hour", onTf }) {
  ...
  const billsTicks = [0, topTickBills * 0.25, topTickBills * 0.5, topTickBills * 0.75, topTickBills];
  const avgBillTicks = [0, topTickAvgBill * 0.25, topTickAvgBill * 0.5, topTickAvgBill * 0.75, topTickAvgBill];

  return (
    <ChartCard>
      <ChartHeader icon={Users} iconColor="#0ea5e9" title="Lưu lượng khách & Bill trung bình">
        <TabToggle value={tf} onChange={onTf} options={[["hour", "Giờ"], ["day", "Ngày"]]} />
      </ChartHeader>
      <BarLineChart
        data={data}
        height={260}
        bars={[{ key: "bills", name: "Số bill", color: chart.skyBar }]}
        lines={[{ key: "avgBill", name: "Bill TB", color: "#f59e0b", format: fmtVND }]}
        barAxis={{ max: billsTicks[4], ticks: billsTicks, formatter: fmtAxisCount }}
        lineAxis={{ max: avgBillTicks[4], ticks: avgBillTicks, formatter: fmtAxisMoney, tooltipFormatter: fmtVND }}
